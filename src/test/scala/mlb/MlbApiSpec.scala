...
object MlbApiSpec extends DefaultRunnableSpec {
  ...
  val mlbApiSuite: Spec[TestEnvironment, TestFailure[Throwable], TestSuccess] =
    suite("MlbApi")(
      testM("create table") {
        for {
  ...
  override def spec: ZSpec[TestEnvironment, TestFailure[Throwable]] =
    mlbApiSuite
}
